  ...
  return (
    ...
    >
      {stats.map((stat, index) => (
        ...
        >
          <div
            className={`text-base sm:text-lg lg:text-xl font-bold font-orbitron mb-2 ${stat.color}`}
          >
            {stat.value}
          </div>
          <div className="text-gray-400 text-xs sm:text-sm flex items-center justify-center gap-2">
            <FontAwesomeIcon icon={stat.icon as IconProp} className="text-xs" />
            {stat.label}
          </div>
        </Card>
      ))}
